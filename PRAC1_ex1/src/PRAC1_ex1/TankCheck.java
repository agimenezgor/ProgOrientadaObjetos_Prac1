...
public class TankCheck{
	...
	public static void main(String[] args){
		...
		do{
			...
			try{//Como se van a capturar errores, necesitamos poner el c�digo que puede lanzar excepciones dentro de "try" y gestionar los posibles errores dentro de "catch"
				...
				switch(operation){
					...
					case 5:
							...
							if(tank.setLength(value)) {
								System.out.println("The tank's length is "+ tank.getLength());
							}else {								
								System.out.println("Length cannot be less than 0.1 cm.!!");								
							} 
			...
			}catch(InputMismatchException e1){//Error que se lanza si el valor, en este caso introducido por teclado, no es un numero.
			...
			}catch(Exception e3){ //Gesti�n del error lanzado por el constructor de la clase Tank que usa los diferentes setters, los cuales pueden lanzar una excepci�n (Ejercicio 2)
			...
			if(operation!=6){				
				...
				try{
					/*
						Como el metodo "read" puede lanzar una excepcion (su implementacion tiene un "throws IOException" (IOException es un caso particular de Exception), ponemos la linea dentro de try
						y gestionamos el error dentro del catch. Si en el catch, en vez de IOException ponemos Exception, tambien funcionara por el concepto de herencia (lo veremos mas adelante en los apuntes).
					*/
					System.in.read();
